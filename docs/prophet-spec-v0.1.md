...
- Paginated query controllers (`GET /<objects>`) plus typed filter queries (`POST /<objects>/query`) and `GET /<objects>/{id}` for single keys or `GET /<objects>/{k1}/{k2}/...` for composite keys
