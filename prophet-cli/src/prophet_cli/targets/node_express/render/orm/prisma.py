...
def _render_prisma_schema(ir: Dict[str, Any], *, provider: str) -> str:
    ...
    for obj in ir.get("objects", []):
        ...
        target_field_names_by_id[oid].add("state")
    ...
    for obj in sorted(ir.get("objects", []), key=lambda item: str(item.get("id", ""))):
        ...
        if obj.get("states"):
            ...
            lines.append(f"  state String{default_hint} @map(\"__prophet_state\")")
    ...
    for obj in sorted(ir.get("objects", []), key=lambda item: str(item.get("id", ""))):
        if not isinstance(obj, dict) or not obj.get("states"):
            continue
        model_name = _pascal_case(str(obj.get("name", "Object")))
        history_model_name = f"{model_name}StateHistory"
        lines.append(f"model {history_model_name} {{")
        lines.append("  history_id Int @id @default(autoincrement())")
        ...
        lines.append("  to_state String")
        lines.append("  occurred_at DateTime @default(now())")
        index_cols = [str(field.get("name", "id")) for field in _object_primary_key_fields(obj)]
        if index_cols:
            lines.append(f"  @@index([{', '.join(index_cols)}], map: \"idx_{_snake_case(model_name)}_state_history_pk\")")
        lines.append("}")
        lines.append("")
    ...
    for obj in sorted(ir.get("objects", []), key=lambda item: str(item.get("id", ""))):
        ...
        for filter_item in query_filters:
            ...
            if field_id == "__state__":
                if "eq" in operators:
                    lines.append(f"  if ({filter_name}Filter?.eq !== undefined) and.push({{ state: {filter_name}Filter.eq }});")
                if "in" in operators:
                    lines.append(f"  if ({filter_name}Filter?.in?.length) and.push({{ state: {{ in: {filter_name}Filter.in }} }});")
                continue
        ...
        if obj.get("states"):
            ...
            lines.append("    const persisted = await this.client.$transaction(async (tx) => {")
            lines.append(f"      const objDelegate = (tx as any).{repo_var};")
            lines.append("      const updateResult = await objDelegate.updateMany({")
            lines.append("        where: { ...primaryWhere, state: expectedState },")
            lines.append("        data: { state: nextState },")
            lines.append("      });")
            lines.append("      if (!updateResult || Number(updateResult.count ?? 0) < 1) {")
            lines.append("        return null;")
            lines.append("      }")
            lines.append(f"      const updated = await objDelegate.findUnique({{ where: {repo_var}UniqueWhere(id) }});")
            lines.append("      if (!updated) {")
            lines.append("        return null;")
            lines.append("      }")
            lines.append(f"      await (tx as any).{_camel_case(f'{obj_name}StateHistory')}.create({{")
            lines.append("        data: {")
            lines.append("          ...primaryWhere,")
            lines.append("          transition_id: transitionId,")
            lines.append("          from_state: expectedState,")
            lines.append("          to_state: nextState,")
            lines.append("        },")
            lines.append("      });")
            lines.append("      return updated;")
            lines.append("    });")
            lines.append(f"    return persisted ? {repo_var}RowToDomain(persisted) : null;")
            lines.append("  }")
        lines.append("}")
        lines.append("")
