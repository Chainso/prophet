...
def _mongoose_schema_type_for_descriptor(
...
def _mongoose_field_config_expr(
    type_desc: Dict[str, Any],
    *,
    required: bool,
    default_value: str = "",
    ...
    *,
    ...
    if kind == "enum":
        ...
        if enum_id in enum_by_id:
            return f"Domain.{_pascal_case(str(enum_by_id[enum_id].get('name', 'Enum')))}"
        return "string"
    if kind == "struct":
    ...
    if kind == "list":
        element = type_desc.get("element", {}) if isinstance(type_desc.get("element"), dict) else {}
        return (
            f"{_ts_type_for_mongoose_document_descriptor(element, type_by_id=type_by_id, enum_by_id=enum_by_id, object_by_id=object_by_id, struct_by_id=struct_by_id)}[]"
        )
    return "unknown"
