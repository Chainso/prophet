...
def _render_action_handlers(ir: Dict[str, Any]) -> str:
    ...
    for action in sorted(ir.get("actions", []), key=lambda item: str(item.get("id", ""))):
        ...
        lines.append(
            f"  handle(input: Actions.{input_name}, context: ActionContext): Promise<ActionOutcomeValue<EventContracts.{output_name}>>;"
        )
        lines.append("}")
        lines.append("")
        ...
        lines.append(f"  async handle(_input: Actions.{input_name}): Promise<ActionOutcome<EventContracts.{output_name}>> {{")
        lines.append(
