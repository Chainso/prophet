...
class TurtleRenderContext:
    ...
    def property_block(self, field: Dict[str, Any]) -> Tuple[str, List[Tuple[str, str]]]:
        ...
        if field.get("is_state_field"):
            statements.append(("prophet:isStateField", _turtle_typed_literal(True, "xsd:boolean")))
            ...
            if enum_id and initial_enum_value_id:
                statements.append(("prophet:initialEnumValue", self.local_resource(initial_enum_value_id)))
...
def render_turtle(ir: Dict[str, Any]) -> str:
    ...
    for event in events:
        ...
        event_subject_by_id[event_id] = event_subject
        ...
        _emit_resource(lines, event_subject, "prophet:Event", statements)
        for field_subject, field_statements in field_defs:
            _emit_resource(lines, field_subject, "prophet:PropertyDefinition", field_statements)
