...
def render_turtle(ir: Dict[str, Any]) -> str:
    ...
    action_outputs = _sorted_items(ir, "action_outputs")
    ...
    action_output_subjects: Dict[str, str] = {}
    if action_outputs:
        lines.extend(
            [
                "# ============================================================",
    ...
    for shape in action_outputs:
        ...
        shape_subject = context.local_resource(shape_id)
        action_output_subjects[shape_id] = shape_subject
        fields = sorted(shape.get("fields", []), key=lambda item: str(item.get("id", "")))
        field_defs = [context.property_block(field) for field in fields]
        statements: List[Tuple[str, str]] = []
        _append_name_description(statements, shape)
        ...
        for field_subject, _ in field_defs:
            statements.append(("prophet:hasProperty", field_subject))
        _emit_resource(lines, shape_subject, "prophet:ActionOutput", statements)
        for field_subject, field_statements in field_defs:
            _emit_resource(lines, field_subject, "prophet:PropertyDefinition", field_statements)
    ...
    for action in actions:
        ...
        statements.extend(
            [
                ...
                ("prophet:producesOutput", context.local_resource(str(action.get("output_shape_id", "")))),
                ("prophet:inLocalOntology", ontology_subject),
            ]
        )
    ...
    for event in events:
        ...
        if event_kind == "action_output":
            output_shape_id = str(event.get("output_shape_id", "")).strip() or event_id
            event_subject_by_id[event_id] = action_output_subjects.get(output_shape_id, context.local_resource(output_shape_id))
            continue
